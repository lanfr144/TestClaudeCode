...
export function PortabilityCard() {
  ...
  const admin = profile?.est_admin_organisation ?? false
  ...
  return (
    ...
    >
      <div className="space-y-4">
        <div>
          <p className="lux-libelle">Droit d’accès — art. 15 et 20 du RGPD</p>
        ...
        {(admin || activeCompanyId) && (
          <div className="border-t border-rule pt-3">
            <p className="lux-libelle">Réversibilité</p>
            <p className="mt-1 text-xs text-ink-muted">
        ...
        <div className="border-t border-rule pt-3">
          <p className="lux-libelle">Référentiel — transmission du savoir</p>
          <p className="mt-1 text-xs text-ink-muted">
            Paramètres légaux datés, barèmes, catalogues et CCT. L’export se lit en clés naturelles
            (<code className="font-mono text-2xs">cle_parametre</code>, codes, dates) et non en identifiants
        ...
        {done && (
          <div className="rounded border border-rule bg-surface-sunken p-2.5">
            <p className="text-xs font-medium text-ink">
              Fichier enregistré · {done.doc.genre} ·{' '}
