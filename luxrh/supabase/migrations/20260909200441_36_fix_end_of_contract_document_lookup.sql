...
begin
    ...
    if (sick->>'days_in_window')::numeric >= excess_days
      ...
      items := items || jsonb_build_object(
        ...
        'detail', fn_fmt((sick->>'days_in_window')::numeric) || ' jours d''incapacité sur '
          ...
          || ' jours, et la protection de ' || (sick->>'protection_weeks') || ' semaines a pris fin.',
  ...
  for r in
    select d.id, d.expires_on, d.employee_id, dt.label as type_label,
  ...
  -- fn_end_of_contract_documents attend l'identifiant du CONTRAT, pas celui de
  -- la fin de contrat.
  for r in
    select c.id as contract_id, ct.notice_end, c.employee_id,
  ...
  loop
    eoc := fn_end_of_contract_documents(r.contract_id);
