-- 77 — Les colonnes à nom composé passent au français
--
-- Deuxième bloc du renommage, après les 55 tables de la migration 76.
-- 271 noms : ceux qui portent un souligné, donc ceux qui ne peuvent venir que
-- de la base. `monthly_gross`, `valid_from`, `certificate_received_at` ne
-- désignent rien d'autre, nulle part. Les 43 noms simples — `name`, `status`,
-- `label` — font l'objet d'un bloc distinct, parce qu'en TypeScript ils
-- désignent aussi bien autre chose.
--
-- CE QUE LA MIGRATION FAIT, DANS L'ORDRE
-- =======================================
-- 1. Renomme les colonnes, partout où le nom apparaît, sans énumérer les tables :
--    la correspondance est jointe au catalogue.
-- 2. Reconstruit les corps de fonctions — même raison qu'en 76 : PostgreSQL les
--    stocke en texte et ils ne suivent pas un renommage.
-- 3. Restitue les droits des fonctions qu'il a fallu supprimer et recréer.
--
-- LE CAS DES FONCTIONS QUI RENVOIENT UNE TABLE
-- =============================================
-- Sept fonctions déclarent `returns table(...)`. Les noms de ces colonnes de
-- sortie font partie du type de retour, et PostgreSQL refuse un
-- `create or replace` qui le modifie. Il faut donc supprimer puis recréer.
--
-- Or une fonction recréée retrouve le `grant execute` implicite à **PUBLIC**.
-- Sans révocation explicite, la migration annulerait en silence la revue de
-- sécurité qui avait ramené à zéro le nombre de fonctions exécutables par un
-- visiteur non authentifié. Le bloc ci-dessous révoque puis restitue exactement
-- les droits relevés avant la suppression.
--
-- Ces noms de sortie sont dans `SORTIES_RPC` de `tools/renommage.py`, séparés des
-- colonnes de table : le catalogue ne les voit pas, mais ce sont les clés JSON
-- que lisent les deux interfaces. Les laisser en anglais donnerait une API à
-- moitié traduite — `cle_parametre` à côté de `gap_days`.
--
-- À FAIRE DANS LE MÊME MOUVEMENT
-- ===============================
--     luxrh-py/.venv/Scripts/python tools/renommer.py colonnes-sures --ecrire
--
-- puis régénérer les types TypeScript et lancer les 190 vérifications.
...
-- 2. Les corps de fonctions, et les droits de celles qu'il faut recreer.
do $fonctions$
declare
  f            record;
  p            record;
  ...
  loop
    ...
    if v_def is distinct from v_avant then
      ...
      exception when others then
        -- Le type de retour a change : create or replace est refuse.
        ...
        -- Une fonction creee retrouve le droit implicite de PUBLIC : on le retire
        -- avant de restituer les droits releves, sinon la revue de securite est
        -- annulee en silence.
