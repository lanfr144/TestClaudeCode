...
console.log('\n== Droit d’accès du salarié ==')
{
  ...
  typeof moi.matricule_national === 'string' && /^\d{13}$/.test(moi.matricule_national)
    ...
    : ko('matricule déchiffré', JSON.stringify(moi.matricule_national))
  'matricule_national_chiffre' in moi
    ? ko('champ chiffré', 'le bytea brut est encore dans la sortie')
    : ok('champ chiffré brut retiré')
  rows(doc, 'contrats') >= 1 ? ok('contrats présents') : ko('contrats', '0')
}
...
console.log('\n== Réversibilité ==')
{
  ...
  const [bg] = await get(manager,
    'societes?select=id,raison_sociale,services!inner(id)&order=raison_sociale&limit=1')
  const [good, doc] = await rpc(manager, 'fn_export_company', { p_company: bg.id })
  good ? ok(`société exportée`, bg.raison_sociale) : ko('export société', JSON.stringify(doc))
  rows(doc, 'salaries') >= 1 ? ok('salariés inclus', `${rows(doc, 'salaries')}`) : ko('salariés', '0')
  rows(doc, 'services') >= 1 ? ok('services inclus') : ko('services', '0')
}
{
  const [good, doc] = await rpc(manager, 'fn_export_organization')
  good ? ok('fiduciaire exportée', `${rows(doc, 'societes')} sociétés`)
...
let referentiel
{
  ...
  rows(doc, 'parametres_legaux') > 100
    ? ok('paramètres légaux', `${rows(doc, 'parametres_legaux')} versions`)
    : ko('paramètres légaux', `${rows(doc, 'parametres_legaux')}`)

  // Les CCT sectorielles ont organisation_id nul : les filtrer sur la seule
  // organisation du demandeur les ferait disparaître de l'export.
  rows(doc, 'conventions_collectives') >= 1
    ? ok('CCT partagées incluses', `${rows(doc, 'conventions_collectives')}`)
    : ko('CCT partagées', 'aucune CCT dans l’export')
  rows(doc, 'regles_convention') >= 1 ? ok('blocs de règles CCT inclus') : ko('règles CCT', '0')

  // Clés naturelles, pas d'identifiants techniques : sinon l'import dupliquerait.
  const p = doc?.payload?.parametres_legaux?.[0] ?? {}
  !('id' in p) && 'cle_parametre' in p && 'debut_validite' in p
...
{
  const journal = await get(manager, 'journal_exports?select=genre_objet,nombre_lignes,taille_octets&order=cree_le.desc&limit=5')
  ...
  journal[0]?.nombre_lignes > 0
    ? ok('volume compté', `${journal[0].nombre_lignes} objets`)
    : ko('volume', `${journal[0]?.nombre_lignes}`)
}
