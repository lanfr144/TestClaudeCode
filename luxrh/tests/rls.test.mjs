...
const [, companies] = await get(admin, 'companies?select=id,legal_name')
const bg = companies.find((c) => c.legal_name.startsWith('Brasserie'))
const [, emps] = await get(admin, `employees?select=id,last_name&company_id=eq.${bg.id}`)
const marta = emps.find((e) => e.last_name === 'Ferreira')
const aicha = emps.find((e) => e.last_name === 'Diallo')
const [, scheds] = await get(admin, `schedules?select=id,status&company_id=eq.${bg.id}`)

console.log('\n== Un autre espace de travail ==')
const other = await signIn(account('other').email, account('other').password)
for (const [label, q] of [
  ['sociétés', 'companies?select=id'],
  ['salariés', 'employees?select=id'],
  ['contrats', 'contracts?select=id'],
  ['plannings', 'schedules?select=id'],
  ['shifts', 'shifts?select=id'],
  ['absences', 'absences?select=id'],
  ['journal d’audit', 'audit_log?select=id'],
...
const [okParams, params] = await get(other, 'legal_parameters?select=id&limit=5')
...
const [, myEmps] = await get(emp, 'employees?select=id,last_name')
myEmps.length === 1 && myEmps[0].id === marta.id
  ? ok('ne voit que sa propre fiche', myEmps[0].last_name)
  : ko('fiches salariés visibles', JSON.stringify(myEmps.map((e) => e.last_name)))

const [, myContracts] = await get(emp, 'contracts?select=id,employee_id')
myContracts.every((c) => c.employee_id === marta.id)
  ? ok('ne voit que ses propres contrats', `${myContracts.length}`)
  : ko('contrats visibles', 'contrats d’autrui exposés !')

const [, myAbs] = await get(emp, 'absences?select=id,employee_id')
myAbs.every((a) => a.employee_id === marta.id)
...
const [, myShifts] = await get(emp, 'shifts?select=id,employee_id')
myShifts.length === 0
  ? ok('aucun shift visible tant que le planning est en brouillon')
  : ko('shifts visibles', `${myShifts.length} shift(s) d’un planning non publié !`)
...
const [okWrite, wErr] = await fetch(`${URL}/rest/v1/absences?employee_id=eq.${marta.id}`, {
