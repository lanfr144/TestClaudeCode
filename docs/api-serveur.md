...
|---|---|---|---|---|
| `fn_param` | `p_key text, p_on date = current_date` | La ligne `parametres_legaux` en vigueur, valeur dérivée résolue | — (socle interne) | 02 |
...
| `fn_referential_gaps` | `p_since date = '2019-12-31'` | Table : `family, cle_parametre, label, couvert_depuis, couvert_jusqua, versions, couvre_depuis, jours_manquants, lu_par`. La colonne `lu_par` et la source `parametres_attendus` datent de la migration 50 (jointure `full outer join` : une clé que le moteur lit mais dont **aucune version n'est chargée** sort avec `versions = 0`, en tête de liste — l'ancienne version, qui groupait `parametres_legaux`, ne pouvait pas la voir, faute de ligne à grouper) | ⚛️ 🐍 | 21 (50) |
| `fn_referential_holes` | — | Table : `cle_parametre, trou_du, trou_au` | ⚛️ 🐍 | 21 |
| `fn_referential_inconsistencies` | `p_on date = current_date` | Table : `cle_parametre, label, published, derived, difference, tolerance, cle_source` | ⚛️ 🐍 | 22 |
| `fn_add_parameter_version` | `p_key text, p_valid_from date, p_value_num numeric, p_value_text text, p_value_json jsonb, p_source text = 'CCSS', p_index_ref numeric, p_note text` | La ligne `parametres_legaux` créée. Clôture la précédente. Réservée à l'administrateur | ⚛️ 🐍 | 21 |
...
| `fn_cba_best_num` | `p_contract uuid, p_block bloc_convention, p_path text, p_on date, p_higher_is_better boolean = true` | `jsonb` — la meilleure valeur parmi toutes les conventions applicables | — | 26 |
| `fn_cba_value` | `p_cba uuid, p_block bloc_convention, p_path text` | `jsonb` — lecture brute d'une clause | — | 02 |
...
| Fonction | Arguments | Retour | Appelée par | Mig. |
|---|---|---|---|---|
| `fn_contract_compliance` | `p_contract uuid, p_on date = current_date` | `jsonb` : contrôles (`code`, `severity`, `detail`, `reference_legale`), mentions obligatoires, arbitrages, `blocking_count`, `can_validate` | ⚛️ 🐍 | 05 (30, 31) |
...
| `fn_overtime_approve` | `p_request uuid, p_as_hr boolean` | La ligne `demandes_heures_sup`. L'accord n'est acquis qu'après validation RH **et** acceptation du salarié | 🐍 | 41 |
...
| `fn_absence_entitlement` | `p_type uuid, p_on date = current_date` | La ligne `droits_absence` en vigueur à la date | — | 33 |
| `fn_disability_extra_leave` | `p_employee uuid, p_on date = current_date` | `jsonb` : `applies`, `taux_pct`, `extra_days` | 🐍 | 40 |
...
| `fn_set_company_rates` | `p_company uuid, p_from date, p_mutuality_class smallint, p_accident_factor numeric, p_activity_class text, p_accident_risk_class text, p_note text` | La ligne `periodes_taux_societe` créée | ⚛️ 🐍 | 23 |
| `fn_employee_sensitive` | `p_employee uuid` | Table `(matricule_national text, iban text)` — **déchiffrement** contrôlé | ⚛️ 🐍 | 03 |
...
| `fn_check_national_id` | `p_id text, p_birth date, p_sex genre_sexe` | `jsonb` : `errors[]` nommant chaque échec — date encodée, parité du sexe, clés de Luhn et de Verhoeff | — | 24 |

## Rémunération et fiscalité

| Fonction | Arguments | Retour | Appelée par | Mig. |
|---|---|---|---|---|
| `fn_income_tax` | `p_taxable numeric, p_class classe_impot, p_on date = current_date, p_periodicity periodicite_impot = 'monthly'` | `jsonb` : tranche, impôt de base, taux sur le dépassement. **`found: false`** tant que `tranches_impot` est vide | — | 37 |
...
| `fn_generate_public_holidays` | `p_year int` | `setof jours_feries` — 11 fériés, fêtes mobiles, collisions et récupérations. **Non exposée** : administration du référentiel | — | 02 (25) |
...
| Fonction | Arguments | Retour | Appelée par | Mig. |
|---|---|---|---|---|
| `fn_request_source` | — | `jsonb` : `ip`, `agent_client`, `identifiant_requete`, lus dans les en-têtes HTTP exposés par PostgREST. Ne lève jamais d'erreur, renvoie des champs nuls hors contexte HTTP | — (interne, révoquée pour tous les rôles y compris `authenticated`) | 49 |
| `fn_log_access` | `p_subject_employee uuid, p_entity_table text, p_entity_id uuid, p_action text, p_scope text = null, p_row_count integer = null` | `void` — insère une ligne dans `journal_acces`, `action` parmi `READ / DECRYPT / EXPORT / DOWNLOAD` | — (interne, révoquée pour tous les rôles ; appelée par `fn_employee_sensitive` et `fn_set_travel_distance`) | 49 |
| `fn_redact_encrypted` | `p_row jsonb` | `jsonb` — remplace la valeur de toute colonne dont le nom finit par `_enc` par `"[chiffré]"` | — (interne, révoquée pour tous les rôles ; appelée par `fn_audit` avant d'écrire dans `journal_ecritures`) | 49 |
| `fn_person_access_report` | `p_employee uuid, p_from timestamptz = null, p_to timestamptz = null` | Table `(quand, qui, qui_id, quoi, detail, nature, d_ou, agent, requete)` — union de `journal_acces`, `journal_ecritures` et `journal_exports` pour une personne. Réservée à ses gestionnaires et à elle-même | — aucun front ne l'appelle encore | 49 |
| `fn_log_access_autonomous` | `p_subject_employee uuid, p_company uuid, p_entity_table text, p_entity_id uuid, p_action text, p_scope text = null, p_row_count integer = 1` | `void` — écrit la trace par `dblink`, hors de la transaction appelante (`est_autonome = true`) ; retombe en écriture dans la transaction courante, en le déclarant (`est_autonome = false`), si `secrets_application.dblink_conninfo` est absent | — (interne, révoquée pour tous les rôles ; appelée par `fn_employee_rows` et `fn_time_entry_rows`) | 51 |
| `fn_employee_rows` | `p_company uuid = null, p_department uuid = null, p_employee uuid = null, p_search text = null, p_active_on date = null, p_fields text[] = ['prenom','nom'], p_limit integer = 500` | `setof employee_row` — lecture pipelinée : chaque ligne est contrôlée (accès société, droit renforcé pour `matricule_national`/`iban`), tracée hors transaction, **puis** émise. Refuse un appel sans `p_company` ni `p_employee` | — aucun front ne l'appelle encore | 51 |
...
**Vérifié sur la base déployée** : `journal_acces` porte déjà deux lignes `action = 'DECRYPT'`,
écrites pendant l'exécution de la suite de tests elle-même, sans qu'aucun code applicatif ait été
modifié pour cela. Les deux portent `est_autonome = false`, faute de `dblink_conninfo` chargée dans
`secrets_application` — la trace survit tant que la transaction appelante n'est pas annulée, pas au-delà.
...
`mileage_allowance_eur_per_km` qu'elles supposent est déclaré dans `parametres_attendus` mais
...
corps du déclencheur `check_address`, posé sur `salaries`, `societes` et `sites_client` — elle
...
| `fn_check_address()` | — (déclencheur) | `trigger` — valide `new`, refuse un verdict `outside` (exception), enregistre le verdict dans `controles_adresse`. Ne revalide que si l'adresse a changé par rapport à `old` ; ne regarde jamais une autre ligne que celle écrite. **Non exposée**, révoquée pour tous les rôles y compris `authenticated` | — (interne, posée sur `salaries`, `societes`, `sites_client`) | 56 |
...
| `est_admin_organisation()` | `boolean` | Administrateur de l'espace |
| `has_role(p_role role_application, p_company uuid)` | `boolean` | Rôle sur une société ou sur tout l'espace |
...
| `has_shift_in_schedule` · `schedule_is_published` | `boolean` | Évitent la récursion entre politiques `plannings` / `creneaux` (migration 18) |
